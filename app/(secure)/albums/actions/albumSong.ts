...
export async function saveExistingAlbumSong(data: AlbumSongSchema): Promise<AlbumSongResponse | ErrorResponse> {  
...
export async function saveNewAlbumSong(data: AlbumSongSchema): Promise<AlbumSongResponse | ErrorResponse> {  
  return await apiCallAuthenticated<AlbumSongResponse>(API_ALBUM_SONG_ADD, API_METHOD.POST, JSON.stringify(mapAlbumSong(data)));
}

export async function deleteAlbumSong(id: number): Promise<AlbumSongResponse | ErrorResponse> {  
