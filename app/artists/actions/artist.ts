...
export async function getRandomArtists(): Promise<ArtistLookup[]> { 
  return await apiGetCall<ArtistLookup[]>(API_GET_RANDOM_ARTISTS, CACHE_TYPE.NO_CACHE);
} 

export async function  saveNewArtistDetails(data: ArtistDetailsSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ARTIST_ADD, API_METHOD.POST, JSON.stringify(mapArtist(data)));
} 

export async function saveExistingArtistDetails(data: ArtistDetailsSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ARTIST_UPDATE, API_METHOD.PUT, JSON.stringify(mapArtist(data)));
}  
...
export async function saveExistingArtistDescriptionDetails(data: ArtistDescriptionSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ARTIST_UPDATE_DESCRIPTION, API_METHOD.PUT, JSON.stringify(mapArtistDescription(data)));
}

export async function getArtist(id: number): Promise<Artist> { 
  return await apiGetCall<Artist>(formatString(API_GET_ARTIST, id), CACHE_TYPE.NO_CACHE);
} 

export async function getArtistsByLetter(letter: string): Promise<ArtistSearchItem[]> {  
  return await apiGetCall<ArtistSearchItem[]>(formatString(API_SEARCH_ARTISTS_BY_LETTER, letter), CACHE_TYPE.NO_CACHE);
}

export async function getArtistsByText(text: string): Promise<ArtistSearchItem[]> {  
  return await apiGetCall<ArtistSearchItem[]>(formatString(API_SEARCH_ARTISTS_BY_TEXT, text), CACHE_TYPE.NO_CACHE);
} 