...
export async function saveExistingAlbumSong(data: AlbumSongSchema): Promise<ApiResponse<AlbumSongResponse>> {  
  return await apiCall<AlbumSongResponse>(API_ALBUM_SONG_UPDATE, API_METHOD.PUT, JSON.stringify(mapAlbumSong(data))); 
}

export async function saveNewAlbumSong(data: AlbumSongSchema): Promise<ApiResponse<AlbumSongResponse>> {  
  return await apiCall<AlbumSongResponse>(API_ALBUM_SONG_ADD, API_METHOD.POST, JSON.stringify(mapAlbumSong(data)));
}

export async function deleteAlbumSong(id: number): Promise<ApiResponse<AlbumSongResponse>> {  
  return await apiCall<AlbumSongResponse>(formatString(API_ALBUM_SONG_DELETE, id), API_METHOD.DELETE, null);   
}