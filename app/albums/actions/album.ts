...
export async function getRandomAlbums(): Promise<AlbumLookup[]> { 
  return await apiGetCall<AlbumLookup[]>(API_GET_RANDOM_ALBUMS, CACHE_TYPE.NO_CACHE);
} 

export async function  saveNewAlbumDetails(data: AlbumDetailsSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ALBUM_ADD, API_METHOD.POST, JSON.stringify(mapAlbum(data)));
} 

export async function saveExistingAlbumDetails(data: AlbumDetailsSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ALBUM_UPDATE, API_METHOD.PUT, JSON.stringify(mapAlbum(data)));
}  
...
export async function saveExistingAlbumDescriptionDetails(data: AlbumDescriptionSchema): Promise<ApiResponse<AddEditActionResponse>> {  
  return await apiCall<AddEditActionResponse>(API_ALBUM_UPDATE_DESCRIPTION, API_METHOD.PUT, JSON.stringify(mapAlbumDescription(data)));
}

export async function getAlbum(id: number): Promise<Album> { 
  return await apiGetCall<Album>(formatString(API_GET_ALBUM, id), CACHE_TYPE.NO_CACHE);
} 

export async function getAlbumsByLetter(letter: string): Promise<AlbumSearchItem[]> {  
  return await apiGetCall<AlbumSearchItem[]>(formatString(API_SEARCH_ALBUMS_BY_LETTER, letter), CACHE_TYPE.NO_CACHE);
}

export async function getAlbumsByText(text: string): Promise<AlbumSearchItem[]> {  
  return await apiGetCall<AlbumSearchItem[]>(formatString(API_SEARCH_ALBUMS_BY_TEXT, text), CACHE_TYPE.NO_CACHE);
} 